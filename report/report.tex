...
POT routine~\cite{pot2021} \code{ot.emd} solves it exactly.
...
fast matrix scaling~\cite{cuturi2013}:
\begin{equation}
  \label{eq:sinkhorn}
  P^\star_\varepsilon
...
Unbalanced OT~\cite{chizat2018} replaces the hard marginal constraints of
\cref{eq:polytope} with soft Kullback--Leibler penalties, so the plan may create
or destroy mass:
\begin{equation}
  \label{eq:unbalanced}
  P^\star
...
Partial OT~\cite{chapel2020} transports only a prescribed fraction
$m \in (0,1]$ of the total mass and is free to leave the rest in place:
\begin{equation}
  \label{eq:partial}
  P^\star
  \;=\; \argmin_{P \ge 0}
...
Sliced OT~\cite{bonneel2015} averages one-dimensional Wasserstein distances over
random projections $\theta \in S^{d-1}$:
\begin{equation}
  \label{eq:sliced}
  \mathrm{SW}_2^2(a,b)
...
\paragraph{Reinhard 2001~\cite{reinhard2001}.} Per-channel
mean/standard-deviation matching in a decorrelated color space (here CIELAB).
...
\paragraph{Iterative Distribution Transfer (Piti\'e 2007~\cite{pitie2007idt}).}
Repeatedly: draw a
...
\paragraph{Monge--Kantorovich Linear / MKL (Piti\'e \& Kokaram 2007~\cite{pitie2007mkl}).}
...
\begin{tikzpicture}[
    font=\small,
    box/.style ={draw, rounded corners=2pt, align=center, minimum height=14mm,
                 minimum width=32mm, inner sep=4pt, fill=blue!5},
    data/.style={draw, align=center, minimum height=14mm, minimum width=26mm,
                 inner sep=4pt, fill=black!6},
    base/.style={draw, dashed, rounded corners=2pt, align=center,
                 minimum height=13mm, inner sep=5pt, fill=orange!12},
    arr/.style ={-{Latex[length=2.6mm]}, thick, rounded corners=2pt},
  ]
  % ---- top row: RGB -> CIELAB -> K-means -> (a,b,C) -------------------- %
  \node[data]                    (rgb) {Source / target\\ RGB image};
  \node[box,  right=20mm of rgb] (lab) {RGB $\to$ CIELAB};
  \node[box,  right=20mm of lab] (km)  {$K$-means\\ $K = 512$, per image};
  \node[data, right=20mm of km]  (abc) {$a,\, b \in \R_+^{K}$\\[3pt]
                                        $C \in \R^{K \times K}$};

  % ---- bottom row: solve_ot -> P -> barycentric -> output ------------- %
  \node[box,  below=42mm of abc]  (solve) {\code{solve\_ot}$(a, b, C, \text{method})$\\[3pt]
        \footnotesize emd $\cdot$ sinkhorn $\cdot$ unbalanced $\cdot$ partial $\cdot$ sliced};
  \node[data, left=20mm of solve] (plan) {transport\\ plan $P$};
  \node[box,  left=18mm of plan]  (bary) {barycentric\\ mapping\ \ $\hat{x}_i$};
  \node[data, left=18mm of bary]  (out)  {Recolored\\ RGB image};

  % ---- baseline box, in the band between the two rows ----------------- %
  \node[base] (bl) at ($ (lab)!0.5!(km) + (0,-21mm) $)
        {Classical baselines (Reinhard / IDT / MKL):\\
         operate directly on CIELAB pixels --- no clustering, no plan};

  % ---- solid OT path ------------------------------------------------- %
  \draw[arr] (rgb) -- (lab);
  \draw[arr] (lab) -- (km);
  \draw[arr] (km)  -- (abc);
  \draw[arr] (abc.south) -- (solve.north);
  \draw[arr] (solve) -- (plan);
  \draw[arr] (plan)  -- (bary);
  \draw[arr] (bary)  -- (out);

  % ---- dashed baseline bypass: CIELAB -> baselines -> output --------- %
  \coordinate (mid) at ($(lab.south)!0.5!(km.south)$);
  \draw[arr, dashed] (mid) -- (bl.north);
  \draw[arr, dashed] (bl.south) -| (out.north);
\end{tikzpicture}}
\caption{The unified color-transfer pipeline. OT methods follow the solid path
(top row left-to-right, then bottom row right-to-left). Classical baselines
follow the dashed path, skipping clustering and the transport plan.}
\label{fig:pipeline}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.72\linewidth]{pareto_plot.png}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.74\linewidth]{mismatch_comparison.png}
...
\begin{itemize}
  ...
  \item Cluster-based barycentric mapping (\cref{eq:bary}) causes spatial speckle
        ...
        (e.g.\ a Laplacian penalty on the map~\cite{ferradans2014}) or a larger
        $K$ with post-hoc smoothing would mitigate this.
